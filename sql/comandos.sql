...
SELECT * FROM BRH.VW_SAUDE_DEPENDENTE;
...
SELECT
    ...
    CASE 
        ...
        WHEN C.SALARIO BETWEEN 3000.01 AND 6000 THEN (C.SALARIO * 0.02)
        WHEN C.SALARIO BETWEEN 6000.01 AND 20000 THEN (C.SALARIO * 0.03)
        ELSE (C.SALARIO * 0.05)
...
-- Criando um relatório que informe os colaboradores que participaram de todos os projetos;

SELECT 
    DISTINCT C.NOME,
    COUNT(A.PROJETO) AS "QUANTIDADE DE PROJETOS"
FROM 
    BRH.ATRIBUICAO A 
INNER JOIN 
    BRH.COLABORADOR C ON C.MATRICULA = A.COLABORADOR
GROUP BY 
    C.NOME 
HAVING 
    COUNT(A.PROJETO) = (SELECT COUNT(*) FROM BRH.PROJETO);